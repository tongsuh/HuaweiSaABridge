# Gadgetbridge - Huawei Band 8 × Sleep as Android (SaA) Edition

> **魔改说明**：本项目基于官方开源 Gadgetbridge 代码，专门针对 **华为手环 8 (Huawei Band 8)** 深度打通与 **Sleep as Android (SaA)** 的双向联动与清醒梦（Lucid Dream）微震提醒功能。

### 🌟 核心魔改特性
1. **华为手环 8 官方 SaA 支持声明**：`HuaweiBand8Coordinator` 正式支持 Sleep as Android，手环可直接出现在 Gadgetbridge 的 SaA 关联设备列表中。
2. **实时 1Hz 心率与体动推流**：响应 SaA `START_TRACKING` 指令，激活华为运动模式连续推流，将 1Hz 心率与体动强度以标准 SaA 广播协议实时转发至 Sleep as Android。
3. **清醒梦微震（REM HINT）参数可调**：在 Gadgetbridge 设置中增加独立的**“清醒梦震动参数设置”**，用户可自由调节**微弱/中等/强力**震动力度、**1~4次**震动次数以及**100~500ms**单次震动时长，无需重新编译。
4. **一键手环震动测试**：在设置界面提供**“测试手环震动”**按钮，戴着手环即可一键实时触发震动，即时感受震动力度与节奏。
5. **智能闹钟长震支持**：SaA 闹钟触发时向手环下发持续强震提醒。
